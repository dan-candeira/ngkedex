...
@Component({
	...
	animations: [
		trigger('cardState', [
			...
			state(
				'Black',
				style({ backgroundColor: '#212121', color: '#eceff1' }),
			),
			state('Soulsilver', style({ backgroundColor: '#b0bec5' })),
			state('Heartgold', style({ backgroundColor: '#ff7043' })),
			state(
				'Platinum',
				style({ backgroundColor: '#673ab7', color: '#ede7f6' }),
			),
			state(
				'Diamond',
				style({ backgroundColor: '#1a237e', color: '#e8eaf6' }),
			),
			...
			state(
				'Sapphire',
				style({ backgroundColor: '#1565c0', color: '#e3f2fd' }),
			),
			...
			state(
				'Blue',
				style({ backgroundColor: '#0d47a1', color: '#e3f2fd' }),
			),
...
export class PokemonDescriptionComponent implements OnInit, OnChanges {
	...
	description?: PokemonDescription | null | undefined;
